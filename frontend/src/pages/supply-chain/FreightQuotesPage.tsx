...
const MOCK_QUOTES: FreightQuote[] = [
  { id: 'fq-001', reference: 'FQ-2026-0891', carrier: 'Maersk Line', origin: 'Mombasa', destination: 'Lagos', mode: 'sea', rate_per_teu: 1850, currency: 'USD', transit_days: 14, valid_until: '2026-03-15', includes_insurance: true, status: 'active', reliability_score: 92 },
  { id: 'fq-002', reference: 'FQ-2026-0890', carrier: 'MSC', origin: 'Mombasa', destination: 'Lagos', mode: 'sea', rate_per_teu: 1720, currency: 'USD', transit_days: 16, valid_until: '2026-03-10', includes_insurance: false, status: 'active', reliability_score: 88 },
  { id: 'fq-003', reference: 'FQ-2026-0889', carrier: 'Kenya Airways Cargo', origin: 'Nairobi (JKIA)', destination: 'Addis Ababa', mode: 'air', rate_per_teu: 4200, currency: 'USD', transit_days: 1, valid_until: '2026-02-28', includes_insurance: true, status: 'active', reliability_score: 97 },
  { id: 'fq-004', reference: 'FQ-2026-0888', carrier: 'Bollore Logistics', origin: 'Mombasa', destination: 'Kampala', mode: 'road', rate_per_teu: 980, currency: 'USD', transit_days: 4, valid_until: '2026-03-31', includes_insurance: true, status: 'active', reliability_score: 82 },
  { id: 'fq-005', reference: 'FQ-2026-0887', carrier: 'CMA CGM', origin: 'Mombasa', destination: 'Dar es Salaam', mode: 'sea', rate_per_teu: 650, currency: 'USD', transit_days: 3, valid_until: '2026-03-20', includes_insurance: false, status: 'active', reliability_score: 94 },
  { id: 'fq-006', reference: 'FQ-2026-0886', carrier: 'SDV Transami', origin: 'Mombasa', destination: 'Kigali', mode: 'road', rate_per_teu: 1450, currency: 'USD', transit_days: 7, valid_until: '2026-02-15', includes_insurance: true, status: 'expired', reliability_score: 78 },
  { id: 'fq-007', reference: 'FQ-2026-0885', carrier: 'Ethiopian Airlines', origin: 'Nairobi (JKIA)', destination: 'Accra', mode: 'air', rate_per_teu: 5100, currency: 'USD', transit_days: 1, valid_until: '2026-03-01', includes_insurance: false, status: 'pending', reliability_score: 95 },
  { id: 'fq-008', reference: 'FQ-2026-0884', carrier: 'PIL', origin: 'Mombasa', destination: 'Durban', mode: 'sea', rate_per_teu: 1200, currency: 'USD', transit_days: 8, valid_until: '2026-03-15', includes_insurance: true, status: 'active', reliability_score: 85 },
];
...
export default function FreightQuotesPage() {
  ...
  const avgRate = (MOCK_QUOTES.filter((q) => q.status === 'active').reduce((s, q) => s + q.rate_per_teu, 0) / MOCK_QUOTES.filter((q) => q.status === 'active').length).toFixed(0);
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          { label: 'Total Quotes', value: MOCK_QUOTES.length.toString(), color: '#D4AF37' },
          { label: 'Active', value: MOCK_QUOTES.filter((q) => q.status === 'active').length.toString(), color: '#22C55E' },
          { label: 'Avg Rate/TEU', value: `$${avgRate}`, color: '#3B82F6' },
          { label: 'Carriers', value: new Set(MOCK_QUOTES.map((q) => q.carrier)).size.toString(), color: '#8B5CF6' },
        ].map((s) => (
      ...
      <Card sx={{ overflow: 'hidden' }}>
        ...
        <Box sx={{ px: 2.5, py: 2, borderTop: '1px solid rgba(212,175,55,0.1)' }}>
          <Typography sx={{ fontSize: 12, color: '#777' }}>
            Showing {filtered.length} of {MOCK_QUOTES.length} freight quotes
          </Typography>
        </Box>
      </Card>
    </Box>
